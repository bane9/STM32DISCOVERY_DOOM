...
static inline void I_CopyFrameBufferRGB565(void)
{
	...
	for(int y = 0; y < SCREENHEIGHT; y++)
	{
		for(int x = 0; x < SCREENWIDTH; x++)
		{
			...
			ST_Framebuffer[x * screen_width + (screen_width - y - 1)]
...
{
	const int screen_height = display_get_width(); // Intentionally flipped
	const int screen_width  = display_get_height();

	const int x_ratio = (SCREENWIDTH << 16) / screen_width;
	const int y_ratio = (SCREENHEIGHT << 16) / screen_height;
	...
	for (int y = 0; y < screen_height; y++)
	{
		...
		for (int x = 0; x < screen_width; x++)
		{
			...
			ST_Framebuffer[x * screen_height + (screen_height - y - 1)]
						   = rgb565_palette[I_VideoBuffer[y2_x2_colors]];
		}
	}
}
